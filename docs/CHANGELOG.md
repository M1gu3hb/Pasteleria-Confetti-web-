...
- **Flags para Miguel (NO tocados):** (1) las **fotos de producto del catálogo** siguen en `media.base44.com` (vienen de `productos.imagen_url`, dato del POS — fuera del alcance de los 8 assets; re-hospedarlas es migración de datos del POS); (2) `creado_por_nombre:"Web Confetti"` lo descarta la whitelist de 0019 (las filas web quedan con `creado_por_nombre=null`) — si se quiere el sello, estamparlo server-side en la RPC; (3) 1 imagen de prueba (93 B) quedó en `web-uploads/pedidos/` (el trigger de Supabase bloquea borrarla por SQL y anon no tiene DELETE) → borrar por Storage dashboard.
