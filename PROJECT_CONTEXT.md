...
- **WEB-2** (port de la capa de datos Base44→Supabase): **HECHO (pendiente de auditoría de Miguel).** Sub-paso 1: RPC `crear_pedido_web` (0019/0020, repo POS). Port: cliente anon + `entitiesAdapter` (contrato `entities.*`), puente/auth/plantilla eliminados (incl. `base44/` y la api_key), NOMBRE→ID verificado con `prueba suscursal`, imágenes de marca re-hospedadas, `UploadFile→web-uploads`. **Build verde + smoke real en preview** (ambos formularios → fila `web/pendiente`, folio del trigger en Gracias, imagen en `web-uploads/pedidos/`; transaccional=0 tras limpiar). Detalle en `docs/CHANGELOG.md` (2026-06-27 cont. 2) y `docs/NEXT_STEPS.md`. Flags: fotos de catálogo aún en `media.base44.com` (`productos.imagen_url`, POS) → bloqueante de cutover documentado en repo POS; `creado_por_nombre` **RESUELTO** (sello server-side, migración 0021); imágenes de prueba residuales en Storage (anotadas para dashboard).
- **WEB-3** (validación end-to-end POS↔web, flujo cruzado): **HECHA y aprobada por evidencia.** FLUJO 1 (pedido web → POS: visible y fiel en PedidosPastel/Caja con badge 🌐 WEB, "Creado por Web Confetti", imagen de referencia; aislamiento por sucursal: Xochimilco no ve los de Topilejo) y FLUJO 2 (edición de producto en el POS reflejada de inmediato en el catálogo web, misma fila vía `catalogo_publico`, sin sync; visibilidad toggle; sin exponer costos). Sin diffs vs Base44. Detalle en `docs/CHANGELOG.md` (2026-06-27 cont. 3). **Siguiente:** import Vercel (Miguel) + el bot de pruebas agresivas (al final). **NO** validar end-to-end POS↔web todavía (eso es WEB-3); pendiente humano: import Vercel.
